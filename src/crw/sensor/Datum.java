...
import crw.proxy.LutraMadaraContainers;
...
enum SENSOR_TYPE {
    ...
    SIMULATED(SENSOR_CATEGORY.ENVIRONMENTAL);
...
public class Datum {
    ...
    public Datum(SENSOR_TYPE type, double[] location, Long timestamp, RealMatrix z, int boatID) { // for environmental data
        this(type,timestamp,z, boatID);
    ...
    public String toString() {
        return String.format("TYPE = %s,  DATE = 20%s,  LAT = %.6e,  LONG = %.6e, VALUE = %s",
                typeString(this.type),df.format(dateobj),location[0],location[1],zString());
    }
